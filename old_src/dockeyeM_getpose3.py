...
if(ipose > 0):
  ...
  print('# of ligand atoms: ',pdb2.natom)
  #
  ...
  for i in range(pdb2.natom):
    # apply ligand rotations and translations
    ...
    for k in range(3):
      xyz[k] = xyz2[k] + gcen2[k] + trn2[k] - gcen1[k] - trn1[k]
    xyz2 = rot_vec(rmt1,xyz,inv=1)
    for k in range(3):
...
while(iline < nline):
  ...
  if(head == 'new min'):
    ...
    for i in range(pdbl.natom):
      ...
      for k in range(3):
        xyz[k] = xyz1[k] + gcenl[k] + trnl[k] - gcen1[k] - trn1[k]
      xyz2 = rot_vec(rmt1,xyz,inv=1)
      for k in range(3):
        xyz[k] = xyz2[k] + gcen1[k]
